...
import type { NowPlayingProps } from "@/common/types/spotify";
...
const NowPlayingCard = ({ isExpand = false }: { isExpand?: boolean }) => {
	...
	const trimmedSongTitle =
		data?.title && data?.title.slice(0, 40) + (data?.title?.length > 40 ? "..." : "");

	const trimmedSongArtist =
		data?.artist && data?.artist.slice(0, 20) + (data?.artist?.length > 20 ? "..." : "");
	...
	return (
		<div className={clsx("z-2 fixed bottom-0 w-full p-3", !expand && "flex justify-end")}>
			{!expand ? (
				<div
					...
					<div className="flex items-center gap-3">
						...
						<div
							...
							onKeyDown={(e) => {
								if (e.key === "Enter") {
									e.preventDefault();
									handleOpenSongUrl(data?.songUrl);
								}
							}}
							role="button"
							tabIndex={0}
