...
- [x] 7.1 启动开发服务器，验证缓存预热日志
- [x] 7.2 测试博客列表 API（分页、搜索、分类）
- [x] 7.3 测试博客详情 API（id 查询、slug 查询）
- [x] 7.4 测试博客详情页数据加载（ISR 模式）
- [x] 7.5 验证 URL 格式为 `/blog/slug-name`（无 ?id=）
- [x] 7.6 检查 Redis 缓存键 `blog:all` 是否存在
- [x] 7.7 验证 API 响应时间（应 < 5ms）
- [x] 7.8 验证 ISR 缓存生效（首次 ~100ms，后续 < 1ms）
- [x] 7.9 运行 lint 和类型检查
- [x] 7.10 测试 Redis 不可用时的降级逻辑（可选）

## 8. 提交与文档

- [x] 8.1 提交代码到 git
- [x] 8.2 编写清晰的 commit message
- [x] 8.3 更新 README（如需要，说明缓存策略）
- [x] 8.4 归档此 OpenSpec 变更
